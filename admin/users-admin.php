		...
		<div class="panel panel-primary">
			...
			<div class="panel-body">
				<div class="col-lg-12" style="padding-left: 0; padding-right: 0;" >
					<div class="pull-left" >
						<a href="add-user-admin.php"><button class="btn btn-success"><span class="glyphicon glyphicon-plus"></span> Add New User</button></a>
					</div>
				</div>
					...
					<table class="table table-striped table-hover table-bordered user">
						...
						for($i=0; $i<count($response['users']); $i++) {?>
							<tr user_id="<?php echo $response['users'][$i]['user_id']; ?>">
								<td><?php echo $response['users'][$i]['first_name'].' '.$response['users'][$i]['last_name']; ?></td>
								<td><?php echo $response['users'][$i]['email']; ?></td>
								<td><?php echo $response['users'][$i]['user_status']; ?></td>
								<td><?php echo $response['users'][$i]['updated_by']; ?></td>
								<td><?php echo $response['users'][$i]['updated_on']; ?></td>
								<td>
									<a href="edit-user-admin.php?user_id=<?php echo $response['users'][$i]["user_id"]; ?>">
