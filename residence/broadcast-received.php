...
$url = ROOT_PATH.'v1/getAssignedSpsBroadcastOrIndividualForRes/broadcast/'.$res_user_id;
        ...
        <div class="row">
            <div class="col-xs-12">
				...
				if($response['error'] != 1)
				{
					...
					{
						for($i=0; $i<count($response['sps']); $i++) {
							$sent_time = $response['sps'][$i]['sent_time']- $tzOffset*60;
							$sent_time = date("d-M-Y H:i:s", $sent_time);
							$avatar = $response['sps'][$i]['image_url'];
							$orgName = $response['sps'][$i]['organisation_name'];
							$title = $response['sps'][$i]['message_title'];
							$description = $response['sps'][$i]['description'];
							$event = $response['sps'][$i]['event'];?>
