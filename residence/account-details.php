...
<script type="text/javascript">
	...
	function OnSuccessFunctionPostcodeLoad(response){
		$(".select-postcode").html("");
		var options = '<select class="postcode-select" id="postcode" name="postcode">';
		options += '<option data="0" value="0">Select Postcode</option>';
		
		var postcode = '<?php echo $postcode; ?>';
		
		$.each(response.postcodes , function(key , value){ // First Level
			var selected = '';
			if(value.postcode_name == postcode){
				selected = 'selected="selected"';
			}
			options += '<option data="'+value.postcode_id+'" value="'+value.postcode_name+'" '+selected+'>' + value.postcode_name + '</option>';
			    
		});
		options += '</select>';
  		$(".select-postcode").append(options);
