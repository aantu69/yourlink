...
//		}
	...
	public function getSpsWithUnreadMsgForApp($category_id, $app_user_id, $timestamp) {
		...
		if ($no_of_rows_user > 0) //if data exist
		{
			...
			{
				$user_id = $row['user_id'];
				$message_results = $this->conn->query("SELECT message_id FROM messages WHERE sent_time > '".$sent_time."' AND receiver_id = 0 AND (sender_id <> $app_user_id AND sender_id = $user_id)");
				$no_of_msg = $message_results->num_rows;
				$rows[] = array(
					'user_id'=> $row['user_id'],
					'user_role'=> $row['user_role'], 
...
";
    ...
    public function shareMessage($message_id, $shared_description, $shared_by, $shared_to_ids) {
    	$message_type = "shared";	
    	$sender_id = $shared_by;
    	$sender_role = 1;	
		$event = 0;
		...
		$messages = $this->getMessage($message_id);
        $num_rows_messages = $messages->num_rows;
        if ($num_rows_messages > 0) //if data exist
		{
		...
		$stmt = $this->conn->prepare("INSERT INTO messages(sender_id, sender_role, receiver_id, message_title, description, event, sent_time, status, message_type, message_sender) values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
		if(count($shared_to_idsArray)>0){
			foreach($shared_to_idsArray as $receiver_id){
				if($receiver_id != ''){
					$stmt->bind_param("iiississsi", $sender_id, $sender_role, $receiver_id, $shared_description, $description, $event, $sent_time, $status, $message_type, $message_sender);
					$result = $stmt->execute();					
				}
			}
		}
...
//    }
    ...
    public function getIndividualMessage($sender_id, $receiver_id, $page) {
    	if($page == 0){
			$stmt = $this->conn->prepare("SELECT *,UNIX_TIMESTAMP(sent_time) AS timestamp FROM messages WHERE message_id NOT IN(SELECT message_id from messages_delete WHERE user_id = ?) AND ((sender_id = ? AND receiver_id = ?) OR (sender_id = ? AND receiver_id = ?)) ORDER BY sent_time");
			$stmt->bind_param("iiiii", $receiver_id, $sender_id, $receiver_id, $receiver_id, $sender_id);
		}else{
			...
			$sql = "SELECT *,UNIX_TIMESTAMP(sent_time) AS timestamp FROM messages WHERE message_id NOT IN(SELECT message_id from messages_delete WHERE user_id = ?) AND ((sender_id = ? AND receiver_id = ?) OR (sender_id = ? AND receiver_id = ?)) ORDER BY sent_time LIMIT ?, ?";
	        $stmt = $this->conn->prepare($sql);
			$stmt->bind_param("iiiiiii", $receiver_id, $sender_id, $receiver_id, $receiver_id, $sender_id, $start, $per_page);
			...
			while ($message = $result->fetch_assoc()) {
				...
				//if(!$this->checkMessageDeleted($message["message_id"], $receiver_id)){
					...
					if($message_type == "shared"){
						$user_info = $this->getUserInfo($message_sender);
				        $num_rows_user_info = $user_info->num_rows;
				        if ($num_rows_user_info > 0) //if data exist
						{
							while ($user = $user_info->fetch_assoc()){
								$tmp["sp_name"] = $user["organisation_name"];
								$tmp["sp_address"] = $user["address"];
								$tmp["sp_phone"] = $user["phone"];
							}
						}
					}
    ...
    public function getBroadcastForAppUser($app_user_id, $event, $timestamp) {
        ...
        $sql = "SELECT u.*,m.* FROM users u INNER JOIN  messages m ON u.user_id = m.sender_id WHERE m.message_id NOT IN(SELECT message_id from messages_delete WHERE user_id = $app_user_id) AND m.receiver_id = 0 AND m.event = $event AND ((u.postcode = '$postcode') OR m.sender_id IN(SELECT sp_id FROM appuserssps WHERE app_user_id = $app_user_id AND active = 1) OR m.sender_id IN(SELECT residence_id FROM residencesindividuals WHERE individual_id = $app_user_id AND active = 1)) ORDER BY m.sent_time";
	...
	public function getSentBroadcastForAppUser($sender_id, $app_user_id, $event) {
        ...
        $response["error"] = false;		
        $already_linked = 0;
        if($this->isThisSpAlreadyAdded($app_user_id, $sender_id))
		{
			$already_linked = 1;
		}
        ...
        if ($num_rows_user_info > 0) //if data exist
		{
			while ($user = $user_info->fetch_assoc()){
				...
				$response["already_linked"] = $already_linked;
			}
		}
...
//		$num_rows_shared = $results_shared->num_rows;
        ...
        $sql = "SELECT * FROM messages WHERE message_id NOT IN(SELECT message_id from messages_delete WHERE user_id = $app_user_id) AND receiver_id = 0 AND sender_id = $sender_id";
		$results = $this->conn->query($sql);		
		$num_rows = $results->num_rows;
		//$this->conn->close();
...
//		}
	...
	public function searchBroadcastForAppUser($app_user_id, $event, $query) {
    	$searchQuery = "%".$query."%";		        
        $sql = "SELECT u.*,m.* FROM users u INNER JOIN  messages m ON u.user_id = m.sender_id WHERE m.message_id NOT IN(SELECT message_id from messages_delete WHERE user_id = $app_user_id) AND m.receiver_id = 0 AND m.event = $event AND (u.postcode LIKE '".$searchQuery."' OR u.organisation_name LIKE '".$searchQuery."' OR m.message_title LIKE '".$searchQuery."') AND (m.sender_id IN(SELECT sp_id FROM appuserssps WHERE app_user_id = $app_user_id) OR m.sender_id IN(SELECT residence_id FROM residencesindividuals WHERE individual_id = $app_user_id)) ORDER BY m.sent_time";
		$results = $this->conn->query($sql);		
		$num_rows = $results->num_rows;
...
//    }
    ...
    public function getSpsMessage($sender_id, $receiver_id) {
		...
		$stmt = $this->conn->prepare("SELECT * FROM messages WHERE message_id NOT IN(SELECT message_id from messages_delete WHERE user_id = ?) AND event = 0 AND sender_id = ?");		
		$stmt->bind_param("ii", $receiver_id, $sender_id);
        $stmt->execute();
        $result = $stmt->get_result();
		...
		$already_linked = 0;
        if($this->isThisSpAlreadyAdded($receiver_id, $sender_id))
		{
			$already_linked = 1;
		}
		$response["already_liked"] = $already_liked;
    ...
    public function getResidencePostcodeWise($postcode) {
		$stmt = $this->conn->prepare("SELECT * FROM users WHERE user_role = 3 AND user_status = 1 AND postcode = ?");		
		...
		if ($num_rows > 0) //if data exist
		{
			$response["residence"] = array();
			while ($residence = $result->fetch_assoc()) {
				$tmp = array();
				$tmp["residence_id"] = $residence["user_id"];
				$tmp["residence_name"] = $residence["organisation_name"];
				array_push($response["residence"], $tmp);
							
			}
		}
		else {
			$response["error"] = true;
			$response["message"] = "No residence available";
		}
        $stmt->close();
        return $response;
    }
